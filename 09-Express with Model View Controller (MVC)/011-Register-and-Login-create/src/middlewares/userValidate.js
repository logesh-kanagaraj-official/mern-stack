...
export const userValidation = async (req, res, next) => {
  ...
  const rules = [
    body("name")
      ...
      .withMessage("Name is required minimum 5 to 20 characters."),
    body("email").isEmail().withMessage("Enter a valid email."),
    body("password")
      .isLength({ min: 8, max: 30 })
      .withMessage("Password is required minimun 8 to 30 characters."),
  ];

  // now run and checking all rules
  await Promise.all(rules.map((rl) => rl.run(req)));
